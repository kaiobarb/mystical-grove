...
uniform float _DisplacementAmt;
    ...
    struct appdata
           ...
           { 
            ...
            float highFreq = -.8 * turbulence( 0.8 * (v.vertex.xyz + timeVec) );
          ...
          //  o.vertex = UnityObjectToClipPos(float4(v.vertex.xyz + displaceAlongNormal, 1.0));
             ...
             float4 newPosition = float4((v.vertex.xyz + displaceAlongNormal).xyz, 1.0);
      
                
                o.noise1 = mul(unity_ObjectToWorld, newPosition); //Vertex position in WORLD coords
          ...
          //   float4 finalColor = float4(i.noise2,i.noise2,i.noise2,1.0);
            ...
            //s return float4( color.r, color2.gb, 0.0 );  
                ...
                //return float4(lerp(cloud, lightColor, 0.2), 0.5);     
				  return float4(cloud*_WorldSpaceLightPos0.y,1);
